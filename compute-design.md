...
-   Cluster-Autoscaler: With the cluster-autoscaler add-on, the worker pools in ROKS on {{site.data.keyword.Bluemix_notm}} VPC cluster can be scaled automatically to increase or decrease the number of worker nodes in the worker pool based on the sizing needs of the scheduled workloads.
